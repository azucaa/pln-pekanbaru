-- Database PLN Pekanbaru (PostgreSQL)
...
CREATE TABLE admin (
    ...
    no_hp VARCHAR(20),
    level VARCHAR(20) DEFAULT 'operator' CHECK (level IN ('admin', 'operator')),
...
CREATE TABLE pemadaman (
    ...
    deskripsi TEXT,
    status VARCHAR(20) NOT NULL CHECK (status IN ('darurat', 'gangguan', 'terencana', 'terdampak')),
    ...
    petugas VARCHAR(100),
    no_tiket VARCHAR(50),
    status_pekerjaan VARCHAR(20) DEFAULT 'menunggu' CHECK (status_pekerjaan IN ('menunggu', 'proses', 'selesai', 'dibatalkan')),
...
CREATE TABLE pelanggan_terdampak (
    id SERIAL PRIMARY KEY,
    pemadaman_id INT NOT NULL REFERENCES pemadaman(id) ON DELETE CASCADE,
    no_pelanggan VARCHAR(20) NOT NULL,
    nama_pelanggan VARCHAR(100),
    alamat TEXT,
    lat DECIMAL(10, 8),
    lng DECIMAL(11, 8),
    status VARCHAR(20) DEFAULT 'terdampak' CHECK (status IN ('terdampak', 'pulih')),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE notifikasi (
    ...
    judul VARCHAR(200) NOT NULL,
    pesan TEXT NOT NULL,
    tipe VARCHAR(20) DEFAULT 'info' CHECK (tipe IN ('info', 'warning', 'danger', 'success')),
    target VARCHAR(20) DEFAULT 'semua' CHECK (target IN ('semua', 'area', 'pelanggan')),
    area_id INT REFERENCES area(id) ON DELETE SET NULL,
...
('PAD-2024-001', 'Pemadaman Terencana Maintenance', 'Pemeliharaan rutin trafo dan jaringan listrik di wilayah Sail', 'terencana', 1, NOW() + INTERVAL '2 days', NOW() + INTERVAL '2 days' + INTERVAL '4 hours', 240, 0.5071, 101.4478, 500, 'Jl. Sudirman, Kel. Sail, Kec. Tenayan Raya', 150, 'Tim Maintenance A', 'TKT-2024-001', 'menunggu', 1),
('PAD-2024-002', 'Gangguan Jaringan Tegangan Menengah', 'Gangguan pada jaringan tegangan menengah akibat cuaca buruk', 'gangguan', 3, NOW() - INTERVAL '2 hours', NULL, 180, 0.5123, 101.4567, 800, 'Jl. Tuanku Tambusai, Kel. Perhentian Marpoyan', 320, 'Tim Emergency B', 'TKT-2024-002', 'proses', 1),
('PAD-2024-003', 'Darurat Kebakaran Gardu Listrik', 'Kebakaran di gardu listrik memerlukan pemadaman darurat', 'darurat', 7, NOW() - INTERVAL '30 minutes', NULL, 360, 0.4987, 101.4389, 1200, 'Jl. Nangka, Kel. Tangkerang Utara', 580, 'Tim Darurat C', 'TKT-2024-003', 'proses', 1),
('PAD-2024-004', 'Pemadaman Terencana Penggantian Trafo', 'Penggantian trafo distribusi yang sudah tua', 'terencana', 5, NOW() + INTERVAL '5 days', NOW() + INTERVAL '5 days' + INTERVAL '6 hours', 360, 0.5234, 101.4654, 400, 'Jl. Melati, Kel. Sidomulyo Timur', 95, 'Tim Maintenance D', 'TKT-2024-004', 'menunggu', 1);
...
('Info Pemadaman Terencana', 'Akan ada pemadaman terencana pada 2 hari mendatang di wilayah Sail. Mohon persiapkan diri Anda.', 'info', 'semua', TRUE, 1, NOW() + INTERVAL '7 days'),
('Peringatan Cuaca Buruk', 'Diperingatkan cuaca buruk yang dapat menyebabkan gangguan listrik. PLN siaga 24 jam.', 'warning', 'semua', TRUE, 1, NOW() + INTERVAL '3 days');
